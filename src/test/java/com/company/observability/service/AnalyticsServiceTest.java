...
class AnalyticsServiceTest {
    ...
    void getRunExecutionsByName_cacheHit_skipsDb() {
        ...
        RunPerformanceData result = service.getRunExecutionsByName("cap", 30, Frequency.DAILY, null, LocalDate.now(), false);
    ...
    void getRunExecutionsByName_cacheMiss_queriesDbAndPopulatesCache() {
        ...
        service.getRunExecutionsByName("cap", 30, Frequency.DAILY, null, LocalDate.now(), false);
    ...
    void getRunExecutionsByName_runNumberDistinguishesKeys() {
        ...
        service.getRunExecutionsByName("cap", 30, Frequency.DAILY, "2", LocalDate.now(), false);
    ...
    void getRunExecutionsByName_blankRunNumber_normalisedToNull() {
        ...
        service.getRunExecutionsByName("cap", 30, Frequency.DAILY, "  ", LocalDate.now(), false);
    ...
    void getRunExecutionsByName_multiAlias_mergesRunsFromAllRealCalculators() {
        ...
        RunPerformanceData result = aliasService.getRunExecutionsByName("capital", 30, Frequency.DAILY, null, LocalDate.now(), false);
    ...
    void getRunExecutionsByName_singleAlias_queriesRealNameOnly() {
        ...
        aliasService.getRunExecutionsByName("portfolio", 30, Frequency.DAILY, null, LocalDate.now(), false);
    ...
    void getRunExecutionsByName_nocache_skipsCacheAndFetchesFromDb() {
        ...
        service.getRunExecutionsByName("cap", 30, Frequency.DAILY, null, LocalDate.now(), true);

        // Cache read must NOT be called — nocache=true skips it entirely
        verify(cacheService, never()).getFromCache(any(), any(), any(), anyInt(), any(), any(), any());
        // DB must be queried regardless
        verify(calculatorRunRepository).findRunsByName(eq("cap"), eq(Frequency.DAILY), eq(30), isNull(), any(LocalDate.class));
        // Fresh result must still be written back to cache
        verify(cacheService).putInCache(eq("executions"), eq("cap"), eq("DAILY"), eq(30), isNull(), any(LocalDate.class), any());
    }
    ...
    void getRunExecutionsByName_referenceLinesUsesStoredSlaTimeWhenPresent() {
        ...
        RunPerformanceData result = service.getRunExecutionsByName("Portfolio", 30, Frequency.DAILY, null, LocalDate.now(), false);
    ...
    void getRunExecutionsByName_emptyRuns_returnsZeroedResponse() {
        ...
        RunPerformanceData result = service.getRunExecutionsByName("cap", 7, Frequency.DAILY, null, LocalDate.now(), false);
