...
2. `observability.sla.live-detection.enabled` will be `true` in any environment where Phase 1 prediction is wanted (prediction reads the `obs:sla:*` structures that registration maintains). Base `application.yml` currently has it `false`. **Updated 2026-09-06:** this is now a *single* opt-in flag gating both Redis registration and the scan job — the former `observability.sla.live-tracking.enabled` was deleted, so prediction can no longer be given deadline data without also running live detection. If Phase 1 ever needs registration *without* the scan job, that split must be re-introduced deliberately (a nested `LiveTracking` in `SlaProperties` plus a second key in the job's `@ConditionalOnProperty`).
