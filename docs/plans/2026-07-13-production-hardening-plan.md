...
**Files:** `RedisCacheConfig.java`, `application.yml`, `application-local.yml`, `application-dev.yml`, `application-prod.yml`, `pom.xml`, `SlaProperties.java`, `RunIngestionService.java`, `SlaMonitoringCache.java`, `LiveSlaBreachDetectionJob.java` (sub-item 5), `src/test/resources/application-test.yml`
...
5. ~~**Live-SLA flag hygiene (user: intentionally off):** move `liveTrackingEnabled` reads into `SlaProperties`…~~ **✅ DONE 2026-09-06 — delivered wider than planned.** The as-planned version (move the `@Value` into `SlaProperties`, add a startup WARN) was implemented first, but a re-pass of `LiveSlaBreachDetectionJob` showed the two-flag design itself was the defect, so the task was extended:

   - **Collapsed two flags into one.** `observability.sla.live-tracking.enabled` is **deleted**; `observability.sla.live-detection.enabled` now gates the whole feature — Redis registration in `SlaMonitoringCache` (via `SlaProperties.liveDetection`) *and* the job bean (`@ConditionalOnProperty`, single key, `matchIfMissing` now `false`). Rationale: two booleans gave four states, two of which are nonsense (populate a set nothing reads; scan a set nothing fills). The third combination — DB-sweep-only detection — already happens automatically whenever Redis is empty or unavailable, so it never needed a knob. The old condition ANDed both keys, which meant `live-tracking=false` silently vetoed `sweepOverdueRunsFromDb`, a mechanism that reads Postgres and never touches Redis.
   - **Default flipped to opt-in (`false`).** The old `@Value(":true")` default contradicted every shipped config (`false` everywhere). The field default and the bean condition now agree: absent = off, one rule. Pinned by `SlaPropertiesTest.liveDetectionEnabled_defaultsToFalseWhenKeyAbsent`.
   - **Startup WARN** lives in `SlaMonitoringCache.warnIfLiveDetectionDisabled` (`@PostConstruct`) — emitted as structured k=v (`event=sla.live_detection.disabled outcome=degraded reason=… impact=hung_runs_do_not_breach_until_completion`) rather than the prose string above. It must live here, not in the job: a bean switched off by `@ConditionalOnProperty` cannot announce its own absence. `SlaMonitoringCacheStartupWarningTest` boots a tiny context to prove the `@PostConstruct` actually fires (a `javax`-vs-`jakarta` import mistake compiles and silently never runs).
   - **Redundant guard deleted.** `RunIngestionService` no longer reads the flag at all — `registerForSlaMonitoring` already rejects on the same flag plus `slaTime == null` and `status != RUNNING`, so the caller's check was pure duplication. Removed the field, the `liveTracking={}` field from the `run.start.persist` log, and 18 `ReflectionTestUtils.setField` calls from `RunIngestionServiceTest` (only one was load-bearing, and it asserts the `slaDeadline == null` path, which is unchanged).
   - **Dead config removed** (found in the same re-pass): `early-warning.enabled` and `early-warning.threshold-minutes` were read by nothing (the early-warning scan has no gate of its own and hardcoded `10`, now the `APPROACHING_WINDOW_MINUTES` constant); the `detectionIntervalMs` `@Value` field in `LiveSlaBreachDetectionJob` was injected and never read (`@Scheduled` uses the placeholder directly).

   **Flag values unchanged in every environment** — both keys were already `false` in `application.yml`/`application-test.yml`, and no profile yml overrode either, so the effective behaviour is identical. ⚠️ **Deployment check:** any external config (Helm values, Azure App Config) that sets `observability.sla.live-tracking.enabled` is now silently ignored; the failure direction is safe (feature stays off), but grep the deploy repo. Docs updated: `CLAUDE.md` SLA section, `docs/plans/2026-07-19-ai-integration-implementation.md` (assumption 2 + Phase-1 demo steps), `docs/plans/sla-live-detection-alerting-epic.md` acceptance criteria.

   Tests: 468 total, 0 failures. The 2 errors are `CalculatorRunRepositoryDimensionalTest` (no local Postgres — Docker down), and 40 Testcontainers tests skipped for the same reason, including `SlaMonitoringCacheIntegrationTest` — **re-run that one with Docker up**, since its `@TestPropertySource` key changed and it has not executed since.
...
Commit: `fix: honor spring.data.redis properties; quiet prod logging; remove dead config` (sub-items 1–4; sub-item 5 shipped separately as `refactor: one opt-in flag for live SLA detection`)
