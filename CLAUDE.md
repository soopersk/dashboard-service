...
| `RunQueryController` | `GET /api/v1/calculators/batch/runs` | Dashboard feed — dimensional run state per calculator for a reporting date. `keys` param is pipe-separated `calculator_name` values. Response keyed by `calculatorName`. Regional calculators return one `RunEntry` per `region`; typed calculators per `runType`. `isRerun=true` signals a re-trigger. With `run_number` on a run-number-aware calculator, null-run_number rows are suppressed strictly; NOT_STARTED projections and placeholders are stamped with the requested `run_number`. `Cache-Control` is state-aware: `max-age=5` while any entry is RUNNING/NOT_STARTED (or has no runs yet), `max-age=30` once all entries are terminal; always `private`. |
...
| `obs:profile:{calcName}:{frequency}:{runNumber}` | String | Run_number-scoped profile. Same TTL rules. **Scoping is run-number-aware-only** (`CalculatorNameResolver.isRunNumberAware`, config `observability.calculator.run-number-aware`): for a non-aware calc (or null runNumber) `getProfile` routes to the blended key above — the `'ALL'`/un-numbered slice *is* its exact slice. |
| `obs:profile:{calcName}:{frequency}:{runNumber\|*}:{dim}` | String | Dimension-scoped profile (e.g. region "WMAP"). `*` when runNumber is null (agnostic calc or null runNumber). Warmed nightly as third tier. All three tiers served by `CalculatorProfileService`. |
...
2. **Live** (`LiveSlaBreachDetectionJob`): on `observability.sla.live-detection.interval-ms` (`120000` ms in `application.yml`), queries Redis sorted set for runs past their frozen deadline, marks `sla_breached=true`, fires `SlaBreachedEvent`. Severity is based on minutes past the deadline using the band gap. Early warning runs on `observability.sla.early-warning.interval-ms` (`180000` ms in `application.yml`) with a hardcoded 10-minute look-ahead window. A **DB fallback sweep** (`sweepOverdueRunsFromDb`, `observability.sla.live-detection.db-sweep-interval-ms`, default `120000` ms) backstops Redis: it queries `findOverdueRunningRuns()` directly so an overdue run still breaches even if its Redis deadline entry was lost (eviction, flush, cold cache). Each `markSlaBreach` + publish is wrapped in a per-run `TransactionTemplate`, so `SlaBreachedEvent` keeps AFTER_COMMIT semantics on the live, sweep, and on-completion publication sites.

SLA monitoring now covers **DAILY and MONTHLY** (any run with a derived deadline), controlled by a single opt-in flag `observability.sla.live-detection.enabled` (default: `false`; absent = off). It gates the whole feature — Redis deadline registration in `SlaMonitoringCache` *and* the `LiveSlaBreachDetectionJob` bean (`@ConditionalOnProperty`, which names the key as a raw string because bean conditions are evaluated before property binding). When it is off, `SlaMonitoringCache` logs one startup WARN (`event=sla.live_detection.disabled`) and breaches are graded only at completion.
...
- The daily aggregate table is `calculator_sli_daily` (Java class: `DailyAggregate`, repo: `DailyAggregateRepository`). It stores **sums** (averages computed at read time), is **frequency-aware** (4-col PK incl. `frequency`, V8), and is rebuilt by the nightly `DailyAggregationJob` — **not** written on `completeRun()`. It is now read **only** by `CalculatorProfileService` (frequency-scoped) for the SLA baseline/estimate profile; the former aggregate-backed analytics endpoints (`/runtime`, `/sla-summary`, `/trends`) have been removed. The recompute is **single-pass**: `run_number = COALESCE(run_number, 'ALL')`, so each run lands in exactly one bucket and un-numbered runs collapse to `'ALL'` (= `null` in profiles). `run_number` scoping applies **only to run-number-aware calculators** — agnostic calcs read the blended/`'ALL'` slice (a stray `run_number` on an agnostic calc is nulled at ingestion and preserved in `additional_attributes`, mirroring the dimension guard). Transition is self-heal (no backfill): older fan-out rows age out of the read lookback — MONTHLY blended `total_runs` settles over ~13 months.
